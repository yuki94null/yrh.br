...
## downじゃなければやめる
    $execute unless data storage yrh.br:status player_data."$(UUID)"{state:"down"} run return fail
...
## 蘇生体のヘルスを設定
    function yrh.br:player/status/revive/set_revive_health
...
## スクアッド側もaliveにして、alive countを減らす それに伴った処理を起こす
    function yrh.br:player/status/revive/in_squad with storage yrh.br:player tmp
