...
## なんのmenu?
    ...
    $execute store result storage yrh.br:ui tmp.Success byte 1.0 run data modify storage yrh.br:ui player_data."$(UUID)".radial_menu.id set from entity @n[tag=yrh.br.player.ui.radial_menu.tmp_chest] Items[{Slot:0b}].components."minecraft:custom_data".yrhbr.item.use_radial_menu.id
